          ...
          while ((newlineIndex = buffer.indexOf('\n')) !== -1) {
            ...
            if (line.startsWith('data: ')) {
              try {
                ...
                // In your handleSearch function, add a new case for rate_limit:
                ...
                switch (data.type) {
                  ...
                  case "rate_limit":
                    console.error('🚫 Rate limit hit!', data);
                    setError(`${data.message}\n\n${data.details}`);
                    addStatusMessage(data.message, "error");
                    clearInterval(timeoutChecker);
                    setIsSearching(false);

                    // Show special rate limit alert
                    setProgress({
                      value: 100,
                      message: `⚠️ Rate limit - Collected ${data.total_results} results`,
                      platform: "",
                    });
                    return;
                  ...
                  case "complete":
                    ...
                    setProgress({
                      value: 100,
                      message: data.rate_limit_hit
                        ? `⚠️ Stopped at rate limit - ${data.total_results} results`
                        : `Complete! Found ${data.total_results} results`,
                      platform: "",
                    });
                    addStatusMessage(
                      data.rate_limit_hit
                        ? `⚠️ Rate limit hit - Collected ${data.total_results} results`
                        : `🎉 Dorking search complete! Total: ${data.total_results} results`,
                      data.rate_limit_hit ? "warning" : "success"
                    );
                    clearInterval(timeoutChecker);
                    setIsSearching(false);
                    return;
        ...
        <Fade in={!!error}>
          ...
          >
            ...
            <Typography variant="caption" sx={{ mt: 2, display: 'block', fontStyle: 'italic' }}>
              💡 Tip: Google CSE free tier allows 100 queries/day.
              Consider reducing the number of platforms or using fewer dork categories.
            </Typography>
          </Alert>
        </Fade>
      )}
